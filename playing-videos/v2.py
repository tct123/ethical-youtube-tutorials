...
def main(page: ft.Page):
    ...
    def handle_new_source(e: ft.ControlEvent):
        ...
        video.playlist_add(
            ft.VideoMedia(resource=e.control.value)
        )  # creates a VideoMedia and add it to the playlist
        video.jump_to(
            len(video.playlist) - 1
        )  # jump to the last media of the playlist (which is the one we added)
